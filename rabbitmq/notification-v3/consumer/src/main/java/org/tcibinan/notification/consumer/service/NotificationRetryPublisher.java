...
public class NotificationRetryPublisher {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(NotificationRetryPublisher.class);

    private final RabbitTemplate template;
    private final MessagePostProcessor notificationMessagePostProcessor;

    public NotificationRetryPublisher(final RabbitTemplate template,
    ...
    public void publish(final Notification notification) {
        LOGGER.info("Publishing to retry exchange {}...", notification);
        switch (notification.type()) {
            case EMAIL -> publish(notification,
                    RabbitMQConfiguration.retryExchange,
                    RabbitMQConfiguration.retryEmailRoutingKey);
            case PUSH -> publish(notification,
                    RabbitMQConfiguration.retryExchange,
                    RabbitMQConfiguration.retryPushRoutingKey);
        }
    }
